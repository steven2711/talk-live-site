...
export const useChatStore = create<ChatStoreState>((set, get) => ({
  ...
  connectToVoiceRoom: async (username: string) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        socket.on('connect', async () => {
          try {
            ...
            // Initialize GlobalVoiceRoomManager with temporary user
            ...
            set({
              ...
              currentUser: null, // 🔥 Don't set currentUser yet
              voiceRoomManager,
            })

            // 🔥 ADD THIS: Listen for voice room joined to get real user ID
            socket.on('voice_room_joined', (roomState: VoiceRoomState) => {
              console.log('Joined voice room:', roomState)

              // 🔥 Find the current user in the room state
              const currentUser =
                roomState.speakers.find(s => s.user.socketId === socket.id)
                  ?.user ||
                roomState.listeners.find(l => l.user.socketId === socket.id)
                  ?.user

              if (currentUser) {
                console.log('Found current user in room state:', currentUser)
                set({
                ...
                // 🔥 Update the voice room manager with the real user ID
                voiceRoomManager.updateCurrentUser({
                  id: currentUser.id,
                  username: currentUser.username,
                })
              } else {
                console.error('Could not find current user in room state')
              }
            })
        ...
        socket.on('connect_error', error => {
          ...
          if (retryCount < maxRetries) {
            ...
            console.log(
              `Retrying connection (${retryCount}/${maxRetries}) in ${delay}ms...`
            )
            ...
            reject(
              new Error(
                `Connection failed after ${maxRetries} attempts: ${error.message}`
              )
            )
          }
        })
    ...
    if (!isServerAvailable) {
      console.warn(
        'Server appears to be unavailable, but attempting connection anyway...'
      )
