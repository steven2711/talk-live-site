...
import { VoiceRoomState, VoiceRoomUser, VoiceRoomRole, VoiceRoomBroadcastMessage } from '../types/chat';
...
export class GlobalVoiceRoomManager {
    ...
    try {
      ...
      if (hasAudioSupport) {
        ...
        try {
          ...
          console.warn('Audio stream manager initialization failed, continuing with limited functionality:', audioError);
          // Continue without audio features
        }
      } else {
        console.warn('Audio capabilities not available, voice room will have limited functionality');
      }
  ...
  async startSpeaking(): Promise<void> {
    try {
      ...
      const listenerIds = this.roomState.listeners.map(listener => listener.user.id);
  ...
  async stopSpeaking(): Promise<void> {
    try {
      if (this.voiceBroadcastManager.isActive && this.voiceBroadcastManager.role === 'speaker') {
  ...
  async stopListening(): Promise<void> {
    try {
      if (this.voiceBroadcastManager.isActive && this.voiceBroadcastManager.role === 'listener') {
  ...
  getCurrentUserRole(): VoiceRoomRole | null {
    ...
    const isSpeaker = this.roomState.speakers.some(speaker => speaker.user.id === this.currentUser!.id);
    if (isSpeaker) return VoiceRoomRole.SPEAKER;

    // Check if user is a listener
    const isListener = this.roomState.listeners.some(listener => listener.user.id === this.currentUser!.id);
   ...
   * Add event listener
   */
  on<K extends keyof VoiceRoomManagerEvents>(event: K, listener: VoiceRoomManagerEvents[K]): void {
  ...
  private setupSocketHandlers(): void {
    ...
    this.socket.on('user_role_changed', (userId: string, newRole: VoiceRoomRole) => {
    ...
    this.socket.on('audio_level_update', (update: { userId: string; audioLevel: number }) => {
      this.emit('audioLevelUpdate', update.userId, update.audioLevel);
    });

    this.socket.on('voice_room_broadcast_signal', (message: VoiceRoomBroadcastMessage) => {
  ...
  private setupBroadcastEventHandlers(): void {
    ...
    window.addEventListener('voiceBroadcastStateChange', (event: any) => {
      ...
      console.log('Broadcast state change:', { role, isActive, peerCount, speakerCount });
    });
  }
  ...
  private startAudioLevelMonitoring(): void {
    ...
    this.audioLevelInterval = window.setInterval(() => {
      if (this.audioStreamManager.isActive()) {
  ...
  private emit<K extends keyof VoiceRoomManagerEvents>(event: K, ...args: Parameters<VoiceRoomManagerEvents[K]>): void {
